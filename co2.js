...
(function (global) {
  ...
  // ══════════════════════════════════════════════════════════════════════
  // VALIDATED MODEL ACCURACY — empirical track record on the cited corpus.
  //   Per-model MAE/RMSE/bias (mm/y) vs measured rates over the n=10 in-scope
  //   carbon-steel CO2 cases (20-80 C), regenerated by benchmark/run.js. This
  //   is what lets PitCast say *which* model to trust, not just run five.
  //   DWM-1975 (no scale term) and FreeCorp over-predict heavily-scaled high-T
  //   cases, inflating their MAE; DWM-1995 is the best single fit. Dated
  ...
  /** Which model to trust — validated best-fit on the cited corpus, with the
   *  honest ensemble-spread caveat. opts (optional): { T_C, u_m_s } adds a
   *  regime note. Returns { recommended, ranked, accuracy, basis, caveat,
   *  regime_note }. Grounded in MODEL_VALIDATION — not a hunch. */
  function recommendModel(opts) {
    opts = opts || {};
    var v = MODEL_VALIDATION.perModel;
    var ranked = Object.keys(v).sort(function (a, b) { return v[a].MAE - v[b].MAE; });
    var best = ranked[0];
    var regime = null;
    if (opts.T_C != null) {
      if (opts.T_C >= 80 && (opts.u_m_s == null || opts.u_m_s <= 1)) {
        regime = 'heavily-scaled / hot-stagnant: every model tends to over-predict the multi-day rate — lean on crMin (lower envelope bound).';
      } else if (opts.T_C <= 40) {
        regime = 'low-temperature / film-poor: the ensemble brackets measured rates well here; the spread is genuine uncertainty.';
      }
    }
    return {
      recommended: best,
      ranked: ranked,
      accuracy: v[best],
      basis: MODEL_VALIDATION.basis + ' (n=' + MODEL_VALIDATION.n_in_scope + ', ' + MODEL_VALIDATION.date + ')',
      caveat: 'Lowest validated MAE (' + v[best].MAE + ' mm/y) on the cited corpus — but all five models disagree by ~10x. Treat the ensemble spread (crMin..crMax), not any single number, as the honest uncertainty.',
      regime_note: regime
    };
  }
  ...
  function assess(o) {
    ...
    // attach each model's validated empirical accuracy (its track record on the corpus)
    models.forEach(function (m) { m.validation = MODEL_VALIDATION.perModel[m.id] || null; });
    ...
    return {
      ...
      spread: spread,
      verdict: verdictFor(crMax),
      recommendation: recommendModel({ T_C: T_C, u_m_s: u }),
